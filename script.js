...
function addFilter() {
    f++;
    document.getElementById('filters').innerHTML += '<div><select id="f' + f + '"></select><input type="number" id="fn' + f + '" style="width: 100px;"></input></div>';
    addOptions('f' + f);
}
...
function filterSet() {
    ...
    for (const [key, value] of Object.entries(list)) {
        ...
        var isTrue = i < 21;
        for (i = 1; i <= f; i++)
            if (document.getElementById('fn' + i).value > 0) {
                q = value[document.getElementById('f' + i).value];
                isTrue = isTrue && (q && q >= document.getElementById('fn' + i).value);
            }
        if (isTrue) {
